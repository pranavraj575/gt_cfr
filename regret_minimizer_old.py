...
class RegretMatching(RegretMinimizer):
    ...
    def next_strategy(self):
        ...
        if sum_regrets == 0:
            self.last_strat = {a: 1 / len(self.action_set) for a in self.action_set}
        else:
            self.last_strat = {a: max(self.cum_regrets[a], 0) / sum_regrets for a in self.action_set}
    ...
    def reset(self, warm_start=None):
        if warm_start is not None and self.cum_regrets is not None:
            sum_regrets = sum(max(r, 0.0) for _, r in self.cum_regrets.items())
            ...
            self.cum_regrets = {a: 0.0 for a in self.action_set}
...
class DCFRRegretMatching(RegretMatching):
    ...
    def observe_utility(self, utility):
        ...
        self.cum_regrets = {
            a: (r * self.t**self.alpha / (self.t**self.alpha + 1) if r >= 0 else r * self.t**self.beta / (self.t**self.beta + 1))
...
class PredictiveRegretMatchingPlus(RegretMatchingPlus):
    ...
    def __init__(self, action_set):
        super().__init__(action_set)
        self.prediction = {a: 0.0 for a in self.action_set}
    ...
    def next_strategy(self):
        ...
        if self.last_strat is None:
            ...
            m_dot_x = sum(self.prediction[a] * self.last_strat[a] for a in self.action_set)

        # theta = r^{t-1}+m^{t}-<m^{t},x^{t}>1
        theta = {a: max(self.cum_regrets[a] + self.prediction[a] - m_dot_x, 0) for a in self.action_set}
        sum_theta = sum(v for a, v in theta.items())

        if sum_theta == 0:
            self.last_strat = {a: 1 / len(self.action_set) for a in self.action_set}
        else:
            self.last_strat = {a: v / sum_theta for a, v in theta.items()}
        return self.last_strat.copy()
